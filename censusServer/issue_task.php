	...
	if($dbh) {
		...
		if( $row['id'] == null ) {
			...
			//	Enter data from user into the MySQL database
                        $query = ('INSERT INTO requests (requesterid, workerid, hitid, assignmentid, ip, mac, data, browser, taskid, country) 
                                VALUES (:requester, :worker, :hit, :assignment, :ip, "", :data, "", :task, :country)');

                        $sth = $dbh->prepare($query);

                        $sth->execute(array(':requester'=>$requester, ':worker'=>$worker, ':hit'=>$hit, ':assignment'=>$assignment, ':ip'=>$_SERVER['REMOTE_ADDR'],
                                ':data'=>serialize($_SERVER), ':task'=>$row['id'], ':country'=>$country));
